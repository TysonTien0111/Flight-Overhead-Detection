...
#define OPENSKY_SERVER_NAME "11.22.54.245"
#define OPENSKY_PORT 5000
#define OPENSKY_GET_PATH "/flights"
//#define OPENSKY_GET_PATH "/api/states/all?lamin=37.55&lomin=-122.50&lamax=37.70&lomax=-122.30"
...
#define AWS_POST_PATH "/things/LizaTinku_CC3200Board/shadow"
...
#define CLHEADER2 "\r\n\r\n"
#define UAHEADER "User-Agent: CC3200\r\n"
...
void MAX7219_SendChar(char c, int pos);
...
void FetchAndDisplayFlightData() {
    ...
    UART_PRINT("\n\r[1] Starting OpenSky Fetch...\n\r");

    g_app_config.host = OPENSKY_SERVER_NAME;
    g_app_config.port = OPENSKY_PORT;

    UART_PRINT("[2] Host: %s\n\r", OPENSKY_SERVER_NAME);
    UART_PRINT("[3] Port: %d\n\r", OPENSKY_PORT);

    UART_PRINT("[4] Attempting HTTP connection...\n\r");
//    int openSkySockID = tls_connect(false);
    int openSkySockID = connectToServer();

    if (openSkySockID < 0) {
        UART_PRINT("[ERROR] HTTP connection to OpenSky FAILED\n\r");
        return;
    }

    UART_PRINT("[5] HTTPS socket connection SUCCESS. Socket ID: %d\n\r", openSkySockID);

    new_flight_data = false;

    UART_PRINT("[6] Sending HTTP request to OpenSky...\n\r");
    opensky_http_get(openSkySockID);

    UART_PRINT("[7] HTTP request finished. Closing socket.\n\r");
    sl_Close(openSkySockID);

    if (new_flight_data) {

        UART_PRINT("[8] Flight data received. Updating OLED.\n\r");

        last_displayed_index = 99;
        UpdateFlightText();

//        UART_PRINT("[9] Syncing flight data to AWS IoT...\n\r");
//
//        g_app_config.host = AWS_SERVER_NAME;
//        g_app_config.port = AWS_PORT;
//
...
//
//        if (awsSockID >= 0) {
//            UART_PRINT("[10] AWS TLS connection SUCCESS\n\r");
//            PostFlightData(awsSockID);
//            sl_Close(awsSockID);
//            UART_PRINT("[11] AWS sync complete\n\r");
//        }
//        else {
//            UART_PRINT("[ERROR] AWS HTTPS connection FAILED\n\r");
//        }

        new_flight_data = false;
    }
    else {
        UART_PRINT("[ERROR] No flight data parsed from OpenSky response\n\r");
    }

    UART_PRINT("[12] Fetch cycle complete\n\r");
}
...
void MAX7219_SendChar(char c, int pos)
{
    ...
    MAX7219_Send(pos + 1, pattern);
}

void MAX7219_Init() {
    MAX7219_Send(0x0F, 0x00);
    MAX7219_Send(0x0C, 0x01);
    MAX7219_Send(0x09, 0x00); // decode
...
//    MAX7219_Init();
    ...
    UART_PRINT("Before Adafruit_Init\n\r");
    Adafruit_Init();

    UART_PRINT("Before fillScreen\n\r");
    fillScreen(SKY_BLUE);

    UART_PRINT("Before UpdateFlightText\n\r");
    strcpy(current_callsign, "UA546");
    strcpy(current_altitude, "12000");
    UpdateFlightText();

    UART_PRINT("After UpdateFlightText\n\r");
    ...
    UART_PRINT("Testing OpenSky API...\n\r");
...
void PostFlightData(int iTLSSockID) {
    ...
    UART_PRINT("[AWS] Preparing payload...\n\r");

    // NEW (same payload, just cleaner formatting)
    sprintf(data, "{\"state\":{\"desired\":{\"var\":\"Flight %s detected at %s meters!\"}}}", current_callsign, current_altitude);
    ...
    // NEW (important header)
    strcpy(pcBufHeaders, "User-Agent: CC3200\r\n");
    pcBufHeaders += strlen("User-Agent: CC3200\r\n");
    ...
    UART_PRINT("[AWS] Sending request...\n\r");

    sl_Send(iTLSSockID, acSendBuff, strlen(acSendBuff), 0);

    int recvSize = sl_Recv(iTLSSockID, &acRecvbuff[0], sizeof(acRecvbuff)-1, 0);

    if(recvSize > 0) {
        acRecvbuff[recvSize] = 0;
        UART_PRINT("[AWS] Response received:\n\r%s\n\r", acRecvbuff);
    }
    else {
        UART_PRINT("[AWS] No response or error receiving\n\r");
    }
}
...
static int opensky_http_get(int iTLSSockID){
    ...
    strcpy(pcBufHeaders, UAHEADER);
    pcBufHeaders += strlen(UAHEADER);
    ...
    UART_PRINT("OpenSky response bytes: %d\n\r", lRetVal);
...
//    }
    if(lRetVal <= 0) {
        new_flight_data = false;
        UART_PRINT("OpenSky request failed\n\r");
    }
    else {
        new_flight_data = true;
    }
