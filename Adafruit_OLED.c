...
// PIN_06
#define OLEDCS_BASE       GPIOA1_BASE
#define OLEDCS_PIN        0x80

// PIN_08
#define DC_BASE           GPIOA2_BASE
#define DC_PIN            0x2

// PIN_15
#define RESET_BASE        GPIOA2_BASE
#define RESET_PIN         0x40
...
void writeCommand(unsigned char c)
{
    ...
    MAP_GPIOPinWrite(OLEDCS_BASE, OLEDCS_PIN, 0);
    MAP_GPIOPinWrite(DC_BASE, DC_PIN, 0);
    MAP_SPIDataPut(GSPI_BASE, c);
    MAP_SPIDataGet(GSPI_BASE, &ulDummy);
    MAP_SPICSDisable(GSPI_BASE);
    GPIOPinWrite(OLEDCS_BASE, OLEDCS_PIN, OLEDCS_PIN);
}
//*****************************************************************************
...
void writeData(unsigned char c)
{
    ...
    MAP_SPICSEnable(GSPI_BASE);
    MAP_GPIOPinWrite(OLEDCS_BASE, OLEDCS_PIN, 0);
    MAP_GPIOPinWrite(DC_BASE, DC_PIN, DC_PIN);
    MAP_SPIDataPut(GSPI_BASE, c);
    MAP_SPIDataGet(GSPI_BASE, &ulDummy);
    MAP_SPICSDisable(GSPI_BASE);
    GPIOPinWrite(OLEDCS_BASE, OLEDCS_PIN, OLEDCS_PIN);
...
void Adafruit_Init(void)
{
  ...
  GPIOPinWrite(RESET_BASE, RESET_PIN, 0);	// RESET = RESET_LOW

  for(delay=0; delay<100; delay=delay+1);// delay minimum 100 ns

  GPIOPinWrite(RESET_BASE, RESET_PIN, RESET_PIN);	// RESET = RESET_HIGH
